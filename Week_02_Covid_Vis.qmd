...
```{r}
covid_data_long_new <- covid_data_long %>% pivot_wider(names_from = count_type,values_from = count)
ggplot(covid_data_long_new, aes(x = date, y = Cases)) + geom_point()

covid_data_long |> 
  filter(state == "MA") |> filter(count_type == "Cases") |>
...
ggplot(data = covid_data_long) +
  geom_bar(mapping = aes(x = count_type, y = count, fill = race), stat = "identity")
```
